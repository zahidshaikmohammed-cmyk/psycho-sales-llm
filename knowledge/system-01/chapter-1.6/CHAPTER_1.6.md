...
Mechanistically, attention can alter the gain, timing, or competition among representations without changing the physical stimulus itself. Behavioral evidence may appear as faster responses, better discrimination, reduced distractor influence, or altered errors, but these outcomes can arise through partly different pathways. Individual performance also fluctuates within a person across state and context, so a single attentional score should not be treated as a fixed capacity. Attention is therefore a family-level construct whose precise operation must be inferred from converging measures.
...
Selection can occur before all stimulus information has been fully analyzed, after some semantic processing, or through reciprocal interactions among processing stages. Findings from different paradigms can therefore look contradictory while actually probing different points in the processing stream. Unattended information may influence behavior without becoming reportable, and reportability itself depends on memory and decision demands. A rigorous account specifies the selection criterion, competing alternatives, timing, and outcome measure before making claims about how much information was processed.
...
Focus can be measured through performance costs from peripheral or competing information, but a strong focus score does not establish a single internal mechanism. Focus may reflect spatial narrowing, stronger task-set maintenance, reduced distractor processing, or strategic prioritization. It also varies with expertise and familiarity: a practiced activity can require precise selection while feeling effortless. Within-person changes across fatigue, emotion, task importance, and environmental complexity can be larger than stable individual differences in some settings.
...
Measurement typically uses prolonged monitoring tasks, reaction-time variability, omission errors, false alarms, and physiological or subjective indices. These measures are not interchangeable: an omission may reflect failure to maintain priority, weak sensory evidence, or a conservative response criterion. Sustained attention is also compatible with periodic micro-lapses rather than uninterrupted processing. The useful question is therefore how reliably relevant priority is recovered and maintained over time, under specified signal and state conditions.

Dual-task costs can be decomposed into perceptual encoding, central processing, response selection, motor execution, and strategic prioritization. Two tasks may appear different at the surface yet collide at a shared internal operation. Conversely, similar-looking tasks may interfere less after practice. Resource limitations therefore need not be measured by one scalar capacity. Trade-offs can also be strategic: people may protect one task by allowing errors or delays in the other. Interpretation should identify which component actually competes.
...
Switching can be prepared when a reliable cue indicates the upcoming target or task, reducing reconfiguration demands. Residual costs after preparation are informative because they suggest incomplete preparation, interference from the prior set, or unavoidable processing transitions. Alternation also has a temporal dimension: frequent changes can repeatedly incur small costs even when each individual shift is successful. Performance should therefore be analyzed as a sequence of transitions rather than as a single trait of flexibility.
...
Spatial priority interacts with perceptual resolution, object structure, eye position, and expectations. A cue can increase sensitivity at one location while leaving other information partially available. Spatial selection is also dynamic: attention may spread, contract, move, or be distributed across multiple relevant locations depending on task structure. Consequently, metaphorical descriptions such as a single fixed spotlight are useful only when treated as models, not literal descriptions of the underlying process.
...
Feature selection can bias processing toward a feature value before a target's exact location is known, but feature dimensions differ in discriminability and natural salience. Attention can also combine feature and location constraints, producing conjunctive selection rather than a purely one-dimensional filter. Experimental effects must be separated from ordinary sensory adaptation, perceptual learning, or differences in physical stimulus strength. The attentional contribution is the change in relative priority attributable to relevance or selection settings.

Object-based effects depend on how strongly the scene supports object boundaries and how the task defines relevance. They can be reduced or altered when spatial distance, feature similarity, or object structure changes. This makes object-based attention an interaction between attentional selection and perceptual representation rather than proof of a single object-sized attentional unit. Measurement often compares responses to information within the same object versus across objects while controlling spatial distance and feature differences.
...
Executive attention is especially difficult to isolate because goal maintenance, conflict detection, working memory, response inhibition, and decision processes can occur together. A conflict-related slowing therefore should not automatically be interpreted as a pure executive-attention measure. The construct is most defensible when the analysis identifies how attentional priority is being regulated in service of a goal. It also varies with practice: practiced task rules can reduce the need for deliberate control while leaving the goal structure intact.
...
Endogenous orienting is often measured by cues whose informational value depends on instructions or learned contingencies. The benefit of such cues depends on whether participants use them, whether the cue predicts the target, and how much time is available to prepare. Incorrect expectations can generate costs, demonstrating that endogenous attention is not simply a benefit-producing mechanism. Individual differences in strategy and task understanding can also influence cueing effects without implying a stable attentional trait.

Exogenous effects are often strongest at particular cue-target intervals and can change direction as attention is first drawn toward a location and later redistributed. This temporal profile shows why stimulus-driven orienting should not be treated as one instantaneous event. External events are also interpreted within learned contexts: novelty, relevance, and previous exposure alter their impact. Thus “external” describes the initiating source, not a complete explanation of the resulting attentional trajectory.
...
Overt attention is closely tied to active sensing because movement changes the information available to the sensory system. This coupling creates reciprocal causation: a task goal can guide a movement, while the newly sampled information can update what should be attended next. Eye tracking can measure overt orienting, but gaze position alone cannot reveal all attentional allocation. Covert shifts can occur before or between movements, and some movements can be mechanically driven without reflecting deliberate selection.
...
Covert attention is commonly inferred from performance changes when gaze is held constant, but covert selection can still depend on perceptual quality and competing information. It can prepare processing before an eye movement, support peripheral monitoring, or shift among nonspatial features. Because direct measurement is indirect, conclusions should distinguish attentional effects from changes in expectancy, alertness, or response preparation. Covert attention is a functional property inferred from dissociations between priority and overt orienting.

Orienting is often time-structured: release from one target, movement of priority, and engagement with another can have different costs. Cueing paradigms estimate these stages indirectly, while eye movements provide a more direct measure of overt orienting but not necessarily of the underlying reason. Orienting can also be repeated rapidly as the environment changes. The process is therefore better represented as dynamic reallocation than as a single command to “look there.”
...
Selection effects can be expressed at different processing stages and may be partly distributed. For example, selection can alter sensory discrimination, subsequent working-memory access, or response competition without requiring one central gate. The distinction between selection and enhancement is also important: raising the priority of a target and suppressing a distractor can produce similar behavioral outcomes through different mechanisms. Strong explanations therefore specify what changes, relative to which competitor, at what time.
...
Attentional control is dynamic rather than purely suppressive. A person may need to strengthen a weak target, disengage from a finished target, widen monitoring, or switch priorities rather than inhibit something. Control can be proactive when likely competition is anticipated and reactive when conflict appears. These modes can have different costs and signatures. Measures of control should therefore specify whether the task requires preparation, conflict resolution, updating, or recovery from an attentional error.

Inhibition is difficult to establish because reduced responding can arise from several processes. A distractor may receive less attention because it is spatially separated, because the target is highly prioritized, because expectations exclude it, or because an active inhibitory process suppresses it. Strong evidence requires designs that distinguish these alternatives. Inhibition can also have costs when the previously irrelevant information later becomes relevant, showing that suppressing priority can trade immediate efficiency for later flexibility.
...
Disengagement is influenced by the strength of the existing attentional priority and the relevance of the new target. A previously attended item may be difficult to release because it remains behaviorally important, because it has acquired learned priority, or because the new target is weak. Delayed release therefore cannot automatically be labeled dysfunctional. In experimental work, disengagement is inferred from the extra time or errors associated with moving away from a previously cued or prioritized target.
...
Attentional sets can be learned and maintained across trials, allowing rapid selection without rebuilding priorities from scratch. This efficiency creates hysteresis: recent task history can continue to bias present selection even after the environment changes. Flexibility therefore depends partly on how quickly an old set can be updated. The construct links attention to learning because repeated contingencies can make selection biases durable, while remaining distinct from the broader acquisition mechanisms of Chapter 1.7.
...
Stimulus-driven and goal-driven influences are better understood as interacting sources than as mutually exclusive categories. A feature can be physically prominent and simultaneously task-relevant; alternatively, a salient event can be ignored when the task strongly constrains selection. Bottom-up descriptions are therefore explanatory shorthand about the direction of influence, not proof that cognition is absent. Perceptual properties still have to be represented before they can affect attentional competition.

Top-down attention can operate through expectations, task rules, learned relevance, and prior knowledge. These sources can change both what is selected and how ambiguous information is interpreted. Because expectations can be wrong, top-down influence can increase confidence in an incorrect target as well as improve performance. Experimental interpretation should distinguish genuine attentional modulation from changes in decision criterion, memory, or response strategy that occur at the same time.
...
Capture is operationally defined differently across experiments, making strong general claims difficult. A capture-like cost may reflect an actual shift of attention, slowed disengagement, response competition, or a change in decision strategy. Eye movements, search efficiency, and reaction-time costs can therefore disagree. The most defensible use of the construct specifies the competing stimulus, the current attentional set, the temporal relationship, and the behavioral signature used to infer capture.
...
Salience can be computed from local feature contrast, but human priority also reflects learned value, semantic relevance, and current goals. A physically distinctive stimulus may receive little behavioral priority if it is irrelevant, while a visually ordinary item can become highly prominent because it matches the task. Salience is therefore neither equivalent to importance nor sufficient for predicting attention. It is one property entering a larger competitive system.

Goal-directed capture shows that a rapid attentional shift can be prepared by task relevance. Matching a current attentional set can increase priority even when the stimulus is not physically exceptional. This helps explain why capture debates cannot be settled by asking whether attention is automatic or controlled as a binary question. The same event can be selected rapidly because a goal has configured the system to treat its feature as important.
...
Feature integration is partly a binding problem and partly an attentional organization problem. Illusory conjunctions, search patterns, and object-based effects have been used to study it, but no single experiment proves that attention is the only binding mechanism. Temporal synchrony, spatial proximity, perceptual organization, and prior knowledge can all contribute. The most precise account treats attention as one contributor to forming stable feature-object associations under particular conditions.
...
Visual search reveals how target definition, distractor similarity, set size, spatial organization, and attentional guidance jointly determine performance. Efficient search does not necessarily mean every item was processed equally; it can reflect strong target-distractor separability or effective guidance. Difficult search likewise does not imply that all items receive identical deep processing. Search is therefore an emergent task outcome that provides evidence about selection mechanisms when alternative explanations are controlled.
...
Spatial cueing provides a controlled way to manipulate expected location, but cue validity and timing determine what process is being measured. Very brief cues can trigger rapid orienting, while informative symbolic cues can support deliberate preparation. Costs after invalid cues may reflect reorienting, disengagement, or decision effects rather than a single “attention shift” variable. Cueing is thus both an experimental method and a family of phenomena whose interpretation depends on design.

Dual-task interference can be localized more precisely by varying temporal overlap, response modality, stimulus similarity, and task priority. If two tasks interfere only when particular components overlap, that pattern provides stronger evidence for component-specific constraints than a general claim about limited attention. People may also strategically prioritize one task, creating asymmetric costs. Consequently, a single overall dual-task score can hide important trade-offs and should not be treated as a pure measure of total attentional capacity.
...
Task switching involves both preparation before the switch and residual interference after it. Advance cues can permit partial configuration of the new task, while the old task may remain active enough to compete. Repetition can reduce switching costs through stronger task-set maintenance and learning. Because switching tasks often changes stimuli, responses, memory demands, and goals simultaneously, experimental designs must control these factors before attributing an effect to switching itself.
...
Multitasking outcomes depend on whether component tasks are concurrent, rapidly alternating, or partly automated. Interruptions can be especially costly because they require reorientation and retrieval of the interrupted task state. Real-world multitasking also differs from laboratory dual-task performance because goals, consequences, environmental cues, and self-selected strategies are richer. The term should therefore describe the behavioral arrangement while leaving mechanism-specific claims to more controlled constructs.

Attentional load can be manipulated through discrimination difficulty, working-memory requirements, number of relevant operations, or target-distractor relations. Different manipulations that are all called “high load” need not have the same effect because they engage different processes. Load can also alter the opportunity for distractor processing without determining whether a distractor is consciously perceived. Careful work therefore defines load operationally and tests whether the predicted change in competition actually occurs.
...
Functional capacity limits emerge from competition among specific processes, and different limitations can coexist. A task may leave perceptual processing relatively intact while constraining response selection, or may permit one task to proceed while another is bottlenecked. This is why resource metaphors are useful summaries but poor substitutes for mechanism. Evidence for limitation is strongest when changing one task component selectively changes the performance cost of another.
...
Inattentional blindness can be increased by demanding tracking, counting, discrimination, or other attentional operations, but detection also depends on the unexpected stimulus itself. Meaningful or visually distinctive events may be noticed more often than weak or similar events. Reports after the event can also be affected by memory and response criteria. The phenomenon therefore demonstrates a conditional failure of noticing, not a general inability to process unattended information.
...
Change blindness is especially informative because observers often experience a coherent, stable scene despite limited explicit comparison of all details. Detection improves when attention is directed to the changed region or when a visual transient makes the difference conspicuous. Semantic relevance and expectations also matter. The effect therefore reveals constraints on comparing scene representations rather than proving that no representation of the unchanged scene exists.

The attentional blink varies with the interval between targets, target characteristics, distractor structure, and task demands. Some manipulations reduce the effect, showing that it is not an immutable refractory period. Competing accounts disagree about whether the main limitation is attentional gating, working-memory consolidation, control engagement, or other temporal interactions. The empirical pattern is therefore stronger than any single mechanistic interpretation.
...
Vigilance performance can be separated into sensitivity and response criterion, because an observer may miss more targets either because signals become harder to discriminate or because the decision threshold changes. Time-on-task effects also interact with arousal, motivation, target predictability, and feedback. A rigorous account therefore avoids treating every miss as attentional depletion. The failure is defined functionally by degraded monitoring under prolonged conditions, while its mechanism remains open to analysis.
...
Distraction can involve external stimuli, internal thoughts, competing tasks, or interruptions. Its impact depends on whether the competing activity captures priority, consumes working-memory resources, changes response selection, or simply changes the person's goals. Subjective annoyance and objective performance impairment can diverge. A complete analysis therefore specifies the outcome being disrupted and the mechanism proposed to produce that disruption.

Attentional failure can be decomposed into failure to select, maintain, disengage, control, or recover. The same observable error may be produced by different failures, so diagnosis from behavior alone is uncertain. Comparing conditions that manipulate target relevance, competition, timing, and task demands can help identify the responsible process. This makes the term useful as a classification layer but dangerous when used as a mechanistic explanation without further specification.
...
Mind-wandering is heterogeneous in content and function. An internally generated episode can involve autobiographical memory, future planning, problem solving, fantasy, worry, or spontaneous associations, and the same person can shift among these modes. It can be intentional or unintentional and can vary in how much it decouples from sensory input. Experience sampling is useful but imperfect because retrospective reports can alter the very state being measured. Performance fluctuations provide complementary evidence but cannot identify thought content.
...
Task switching can be decomposed into preparation, disengagement from the previous set, activation of the new rule, and response selection. Advance cues often reduce costs because they permit partial preparation, while residual switch costs can reveal interference from the prior task set. Repetition, practice, and strong cue-task associations can reduce these costs without making switching free. Because switching tasks can also change stimuli and responses, experiments must separate genuine task-set reconfiguration from ordinary changes in perceptual or motor demands.
...
Multitasking can include true concurrent processing, rapid alternation, or a mixture in which one activity is relatively automatic. Interruptions add another demand because the person must recover the state of the interrupted task. Real-world multitasking also contains self-selected priorities, consequences, environmental reminders, and uneven task importance, making it richer than laboratory dual-task paradigms. The everyday label therefore describes an arrangement of behavior, while its underlying mechanisms must be identified separately.
...
Attentional load can be manipulated through discrimination difficulty, the number of relevant operations, working-memory requirements, or competition among target and distractor features. These manipulations are not interchangeable: two tasks described as high-load may rely on different mechanisms. Load can alter the amount of processing available to irrelevant information, but it does not by itself establish whether that information was consciously perceived. Rigorous use of the construct requires specifying the operational manipulation and testing the predicted change in distractor processing.
...
Functional limitations can arise at different points in processing. One task may interfere with another because of shared perceptual analysis, central decision processes, response selection, working-memory maintenance, or executive control. Practice can reduce some bottlenecks, while strategic prioritization can move costs from one task to another. Thus evidence for limited capacity is not evidence for a single measurable pool of attention. The most informative experiments identify which task component creates the constraint and whether changing that component selectively changes interference.
...
Detection depends not only on attention but also on the unexpected stimulus's strength, similarity to attended material, semantic relevance, and timing. Meaningful or distinctive unexpected objects may be noticed more often than weak ones. Post-event questioning can also involve memory and response criteria, so subjective reports are not a direct readout of all processing that occurred during the event. Inattentional blindness therefore demonstrates a conditional failure of noticing under competing demands rather than proving that unattended information receives no processing.
...
Change detection requires a useful comparison between representations across moments. When an interruption removes a transient signal or attention is occupied elsewhere, the observer may fail to establish which feature changed even though the scene appears stable. Detection improves when attention is directed toward the relevant region or when the change produces a strong transient. Expectations, semantic importance, and memory for the original scene also matter. The phenomenon therefore reveals limits on explicit comparison rather than proving that visual scenes are stored with no detail at all.
...
The magnitude of the attentional blink varies with the temporal gap between targets, target and distractor characteristics, task demands, and practice. Some manipulations reduce the effect, showing that it is not an immutable refractory period. Competing explanations emphasize temporary gating, working-memory consolidation, control engagement, or interactions among these processes. The empirical phenomenon is therefore well established while its precise mechanism remains theory-dependent. It should be used as evidence for temporal constraints on sequential target processing, not as proof of one universal attentional bottleneck.
...
Vigilance performance can be separated into sensitivity to signals and the response criterion used to classify uncertain events. A rise in misses can therefore reflect weaker discriminability, altered decision threshold, fluctuating engagement, or several factors together. Time-on-task effects also interact with target frequency, feedback, motivation, arousal, and circadian state. Vigilance failure is consequently a functional description of degraded prolonged monitoring, not a diagnosis of one cause. Signal-detection analysis is often useful precisely because it prevents criterion changes from being mistaken for pure sensitivity loss.
...
Distraction can originate outside the person, inside thought, or from another task. Its behavioral impact depends on whether the competitor captures priority, consumes working-memory operations, creates response conflict, interrupts task-set maintenance, or changes the person's goal. Subjective annoyance can occur without measurable performance loss, while subtle competing information can impair performance without being consciously reported. A precise account therefore identifies the disrupted outcome and the mechanism proposed to produce the disruption rather than treating every unwanted influence as the same form of distraction.
...
Attention failure can be decomposed into failure to select relevant information, maintain engagement, disengage from an obsolete target, regulate competition, or recover after an attentional error. The same behavioral mistake can arise from perception, memory, motivation, knowledge, or decision processes, so the label is not sufficient as a causal explanation. Mechanism-specific analysis should manipulate target relevance, timing, competition, and task demands to determine what changed. Attention failure is therefore most useful as a classification layer that points toward a more precise failure mechanism.
...
Mind-wandering is heterogeneous in both content and function. Internally generated episodes may involve autobiographical memories, future plans, problem solving, imagery, worry, or spontaneous associations, and they can be intentional or unintentional. The degree of sensory decoupling also varies: a person can remain partly responsive to the environment while strongly engaged in internal thought. Experience sampling and thought probes provide direct information about content but can alter or interrupt the state being measured; performance variability supplies complementary but indirect evidence. Mind-wandering can impair current-task monitoring while also supporting planning and reflection, so it is not inherently dysfunctional.
