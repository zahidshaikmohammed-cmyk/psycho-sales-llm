# PSYCHO SALES — SYSTEM 1
# HUMAN PSYCHOLOGY
## SURGICAL CANONICAL ARCHITECTURE v2.0

**Status:** LOCKED — SURGICAL CANONICAL ARCHITECTURE v2.1
**Purpose:** Establish a complete, non-duplicative architecture for universal psychological science before prose is written.
**Core rule:** One canonical owner per construct. Cross-domain relationships are links, not duplicate owners.

---

# I. ARCHITECTURAL DESIGN

## A. System 1 owns
System 1 owns universal psychological science: biological, cognitive, affective, motivational, developmental, personality, social, cultural, behavioral, clinical, health, and methodological principles that can be applied across human contexts.

## B. System 1 does not own
System 1 does not own sales, investor, brokerage, communication engineering, persuasion methodology, negotiation methodology, buyer-specific behavior, organizational implementation, or AI implementation. Those are specialized later systems.

## C. Ontology types

Every leaf object must be classified as one of:
1. Construct
2. Process
3. Mechanism
4. State
5. Phenomenon
6. Individual difference
7. Theory/model
8. Method/measurement object
9. Clinical condition
10. Application/relationship

Only the first eight and clinically necessary objects receive independent canonical ownership. Pure relationships are linked rather than treated as new constructs.

## D. Canonical rules

- A construct has one owner.
- A mechanism has one owner.
- A theory/model is owned where its primary explanatory target lives.
- A developmental manifestation does not re-own the general construct.
- A cultural manifestation does not re-own the general construct.
- A clinical manifestation does not re-own the general mechanism.
- A relationship between two owners is a cross-link, not a third owner.
- Applied domains are not promoted into System 1 unless the principle is genuinely universal.
- Theories with mixed or contested evidence are labeled as such.
- Historical concepts remain only when they materially explain the development of psychological science.
- No chapter exists merely to hold leftovers.

---

# CHAPTER 1.1 — FOUNDATIONS AND ARCHITECTURE OF HUMAN PSYCHOLOGY

### Core ontology
- Psychology
- Behavior
- Mental processes
- Mental states
- Mental contents
- Psychological functions
- Psychological mechanisms
- Psychological systems
- Psychological constructs
- Psychological traits
- Psychological capacities
- Psychological organization
- Human functioning
- Individual differences
- Person
- Situation
- Context
- Person–situation interaction
- Person–environment interaction
- Behavior–environment transaction
- Psychological adaptation
- Psychological plasticity
- Psychological stability
- Psychological flexibility
- Psychological change

### Processing architecture
- Stimulus
- Psychological processing
- Information selection
- Interpretation
- Representation
- Evaluation
- Response selection
- Action
- Feedback
- Self-regulation
- Reciprocal causation
- Dynamic interaction
- Temporal dynamics
- Psychological continuity
- Psychological emergence

### Levels and explanations
- Levels of analysis
- Levels of explanation
- Biological explanation
- Psychological explanation
- Social explanation
- Cultural explanation
- Multilevel explanation
- Biopsychosocial perspective
- Systems perspective
- Dynamic-systems perspective
- Functional explanation
- Structural explanation
- Process explanation
- Mechanistic explanation
- State–trait–process distinction
- Nature–nurture question
- General principles and individual variation

---

# CHAPTER 1.2 — PSYCHOLOGICAL SCIENCE, RESEARCH, MEASUREMENT, AND EVIDENCE

### Science
- Psychological science
- Scientific inquiry
- Basic research
- Applied research
- Scientific theory
- Theory construction
- Hypothesis
- Prediction
- Description
- Explanation
- Scientific inference
- Falsifiability
- Evidence
- Evidence quality
- Scientific uncertainty
- Scientific consensus
- Pseudoscience
- Replication
- Reproducibility
- Generalizability

### Research designs
- Experimental research
- Randomized experiment
- Quasi-experiment
- Correlational research
- Observational research
- Naturalistic observation
- Survey research
- Case study
- Longitudinal design
- Cross-sectional design
- Sequential design
- Experience sampling
- Ecological momentary assessment
- Single-case design
- Qualitative research
- Mixed-methods research

### Variables and inference
- Construct
- Operational definition
- Variable
- Independent variable
- Dependent variable
- Mediator
- Moderator
- Confound
- Control variable
- Causal inference
- Internal validity
- External validity
- Ecological validity
- Statistical conclusion validity
- Construct validity
- Measurement validity
- Correlation
- Correlation versus causation

### Sampling and statistics
- Population
- Sample
- Sampling frame
- Random sampling
- Sampling bias
- Selection bias
- Descriptive statistics
- Central tendency
- Variability
- Distribution
- Correlation coefficient
- Regression
- Statistical power
- Type I error
- Type II error
- Statistical significance
- Effect size
- Confidence interval
- Multiple comparisons
- Bayesian reasoning
- Practical significance

### Psychometrics
- Psychological measurement
- Reliability
- Test–retest reliability
- Internal consistency
- Inter-rater reliability
- Measurement error
- Validity
- Scale construction
- Item analysis
- Norm-referenced assessment
- Criterion-referenced assessment
- Standard scores
- Percentiles
- Psychometrics
- Classical test theory
- Item response theory
- Factor analysis
- Latent variables
- Measurement invariance

### Open science and ethics
- Preregistration
- Registered reports
- Researcher degrees of freedom
- P-hacking
- HARKing
- Selective reporting
- Publication bias
- Citation bias
- Meta-analysis
- Systematic review
- Meta-analytic heterogeneity
- Open science
- Data transparency
- Data sharing
- Peer review
- Researcher bias
- Informed consent
- Confidentiality
- Privacy
- Deception
- Debriefing
- Risk–benefit assessment
- Vulnerable populations
- Animal research ethics

---

# CHAPTER 1.3 — BIOLOGICAL BASES OF HUMAN BEHAVIOR AND MENTAL LIFE

### Neural foundations
- Neuron
- Glial cell
- Neural communication
- Electrical signaling
- Action potential
- Synaptic transmission
- Excitation
- Inhibition
- Neurotransmitter
- Neuromodulation
- Neural plasticity
- Synaptic plasticity
- Myelination
- Neural pruning
- Neural network
- Functional connectivity

### Nervous system
- Central nervous system
- Peripheral nervous system
- Somatic nervous system
- Autonomic nervous system
- Sympathetic nervous system
- Parasympathetic nervous system
- Enteric nervous system
- Spinal cord
- Motor system
- Sensory pathways
- Motor pathways

### Brain
- Brain organization
- Brainstem
- Cerebellum
- Thalamus
- Hypothalamus
- Basal ganglia
- Limbic-system structures
- Cerebral cortex
- Association cortex
- Frontal lobe
- Parietal lobe
- Temporal lobe
- Occipital lobe
- Prefrontal cortex
- Motor cortex
- Somatosensory cortex
- Corpus callosum
- Hemispheric specialization
- Lateralization

### Endocrine, genes, evolution
- Endocrine system
- Hormones
- Neuroendocrine interaction
- Gene
- Genetic variation
- Gene expression
- Heritability
- Gene–environment interaction
- Epigenetic processes
- Evolution
- Natural selection
- Evolutionary adaptation
- Biological constraints

### Regulation and body
- Homeostasis
- Allostasis
- Biological rhythms
- Circadian rhythms
- Brain–body interaction
- Embodiment
- Interoception
- Immune–behavior interaction
- Psychoneuroimmunology
- Reproductive hormones
- Biological individual differences

### Methods
- Neuropsychology
- Brain injury and psychological function
- Neuroimaging
- EEG
- Lesion methods
- Behavioral genetics
- Twin studies
- Adoption studies

---

# CHAPTER 1.4 — SENSATION AND PERCEPTION

### Foundations
- Sensation
- Perception
- Sensory receptor
- Transduction
- Sensory coding
- Receptive field
- Absolute threshold
- Difference threshold
- Signal detection theory
- Signal-to-noise
- Criterion
- Response bias
- Sensory adaptation

### Perceptual processing
- Bottom-up processing
- Top-down processing
- Perceptual inference
- Perceptual organization
- Gestalt organization
- Figure–ground
- Similarity
- Proximity
- Continuity
- Closure
- Common fate
- Perceptual completion
- Perceptual constancy
- Size constancy
- Shape constancy
- Color constancy
- Perceptual set
- Expectation effects
- Context effects
- Perceptual learning
- Perceptual bias

### Vision
- Visual perception
- Color vision
- Visual pathways
- Depth perception
- Monocular cues
- Binocular cues
- Motion perception
- Object recognition
- Pattern recognition
- Face perception
- Biological motion

### Other senses
- Auditory perception
- Auditory pathways
- Vestibular perception
- Somatosensation
- Touch
- Pain perception
- Temperature perception
- Proprioception
- Interoceptive perception
- Olfaction
- Gustation
- Multisensory integration
- Cross-modal perception
- Multisensory conflict

### Phenomena
- Illusions
- Hallucination as perceptual phenomenon
- Individual differences in perception

---

# CHAPTER 1.5 — CONSCIOUSNESS, SLEEP, AND ALTERED STATES

### Consciousness
- Consciousness
- Subjective experience
- Awareness
- Self-awareness
- Levels of consciousness
- Conscious access
- Conscious processing
- Unconscious processing
- Subliminal processing
- Automaticity
- Automatic processing
- Controlled processing
- Consciousness measurement

### Sleep
- Sleep
- Sleep architecture
- Sleep regulation
- Sleep homeostasis
- Circadian sleep regulation
- Sleep pressure
- NREM sleep
- REM sleep
- Sleep and memory
- Sleep and learning
- Sleep and emotion
- Sleep deprivation
- Chronotype
- Circadian misalignment
- Dreams
- Lucid dreaming
- Sleep paralysis

### Altered states
- Hypnosis
- Meditation
- Mindfulness
- Altered states of consciousness
- Dissociation
- Dissociative states
- Psychoactive substances
- Stimulants
- Depressants
- Hallucinogens
- Opioids
- Drug tolerance
- Dependence
- Withdrawal

---

# CHAPTER 1.6 — ATTENTION AND INFORMATION SELECTION

### Attention
- Attention
- Selective attention
- Focused attention
- Sustained attention
- Divided attention
- Alternating attention
- Spatial attention
- Feature-based attention
- Object-based attention
- Executive attention
- Endogenous attention
- Exogenous attention
- Overt attention
- Covert attention
- Orienting
- Attentional selection
- Attentional control
- Attentional inhibition
- Attentional disengagement
- Attentional set

### Selection and control
- Bottom-up attention
- Top-down attention
- Attentional capture
- Salience
- Goal-directed capture
- Feature integration
- Visual search
- Spatial cueing
- Dual-task interference
- Task switching
- Multitasking
- Attentional load
- Resource limitations

### Failures
- Inattentional blindness
- Change blindness
- Attentional blink
- Vigilance failure
- Distraction
- Attention failure
- Mind-wandering

---

# CHAPTER 1.7 — LEARNING AND BEHAVIOR CHANGE

### Learning foundations
- Learning
- Learning from experience
- Behavioral change through learning
- Associative learning
- Biological constraints on learning
- Preparedness

### Classical conditioning
- Classical conditioning
- Unconditioned stimulus
- Unconditioned response
- Conditioned stimulus
- Conditioned response
- Acquisition
- Extinction
- Spontaneous recovery
- Stimulus generalization
- Stimulus discrimination
- Generalization gradient
- Conditioned inhibition
- Higher-order conditioning
- Blocking
- Overshadowing
- Latent inhibition

### Operant learning
- Operant conditioning
- Reinforcement
- Positive reinforcement
- Negative reinforcement
- Punishment
- Positive punishment
- Negative punishment
- Primary reinforcer
- Secondary reinforcer
- Reinforcement schedule
- Fixed-ratio schedule
- Variable-ratio schedule
- Fixed-interval schedule
- Variable-interval schedule
- Shaping
- Chaining
- Discriminative stimulus
- Escape learning
- Avoidance learning
- Reinforcer devaluation

### Social and cognitive learning
- Observational learning
- Modeling
- Imitation
- Vicarious learning
- Social learning
- Cognitive learning
- Latent learning
- Insight learning
- Learning without immediate performance

### Habituation and skills
- Habituation
- Sensitization
- Skill learning
- Practice
- Skill acquisition
- Expertise development
- Transfer of learning
- Learning strategies
- Feedback and learning
- Self-regulated learning
- Behavior modification
- Habit formation through learning
- Habit strength
- Habit disruption
- Learning and context

---

# CHAPTER 1.8 — MEMORY

### Memory architecture
- Memory
- Encoding
- Storage
- Retrieval
- Consolidation
- Reconsolidation
- Levels of processing
- Chunking
- Retrieval cues
- Encoding specificity

### Memory systems
- Sensory memory
- Iconic memory
- Echoic memory
- Short-term memory
- Working memory
- Working-memory capacity
- Central executive
- Phonological loop
- Visuospatial sketchpad
- Episodic buffer
- Long-term memory
- Explicit memory
- Implicit memory
- Declarative memory
- Nondeclarative memory
- Episodic memory
- Semantic memory
- Procedural memory
- Priming
- Conditioning memory
- Prospective memory
- Autobiographical memory
- Flashbulb memory

### Retrieval and forgetting
- Recognition
- Recall
- Relearning
- Context-dependent memory
- State-dependent memory
- Retrieval practice
- Spacing effect
- Testing effect
- Forgetting
- Decay
- Interference
- Proactive interference
- Retroactive interference
- Retrieval failure
- Tip-of-the-tongue state
- Directed forgetting
- Retrieval-induced forgetting
- Serial-position effect
- Primacy effect
- Recency effect

### Memory errors
- Memory distortion
- False memory
- Misinformation effect
- Source monitoring
- Source memory
- Memory reconstruction
- Suggestibility
- Eyewitness memory
- Familiarity
- Recollection
- Memory confidence
- Memory accuracy
- Metamemory
- Memory monitoring
- Memory control

### Clinical and personal memory
- Amnesia
- Anterograde amnesia
- Retrograde amnesia
- Memory and identity
- Emotion and memory
- Stress and memory
- Sleep and memory
- Individual differences in memory

---

# CHAPTER 1.9 — THINKING, REPRESENTATION, REASONING, AND PROBLEM SOLVING

### Representation
- Thinking
- Cognition
- Mental representation
- Concepts
- Categories
- Prototypes
- Exemplars
- Categorization
- Concept formation
- Schemas
- Scripts
- Mental models
- Knowledge structures
- Mental imagery
- Symbolic thought
- Abstract thinking
- Concrete thinking
- Rule learning
- Knowledge representation

### Reasoning
- Reasoning
- Deductive reasoning
- Inductive reasoning
- Abductive reasoning
- Causal reasoning
- Counterfactual thinking
- Probabilistic reasoning
- Logical reasoning
- Analogical reasoning
- Analogical transfer
- Mental simulation
- Prospective thinking

### Problem solving
- Problem solving
- Problem representation
- Problem identification
- Goal states
- Planning
- Means–ends analysis
- Algorithms
- Heuristics
- Insight
- Mental set
- Functional fixedness
- Cognitive fixation
- Creative cognition
- Creativity
- Divergent thinking
- Convergent thinking

### Executive cognition
- Executive functions
- Inhibitory control
- Updating
- Cognitive shifting
- Cognitive flexibility
- Cognitive control
- Metacognition
- Metacognitive knowledge
- Metacognitive monitoring
- Metacognitive control
- Metacognitive experience
- Confidence in thinking
- Cognitive load
- Bounded rationality
- Cognitive reflection
- Belief formation
- Belief revision
- Belief updating
- Knowledge acquisition
- Expertise and thinking

---

# CHAPTER 1.10 — JUDGMENT, DECISION-MAKING, AND CHOICE

### Decision foundations
- Judgment
- Decision-making
- Choice
- Preference
- Preference construction
- Preference reversal
- Decision under certainty
- Decision under risk
- Decision under uncertainty
- Decision under ambiguity
- Expected value
- Expected utility
- Subjective probability
- Satisficing
- Bounded rationality

### Heuristics
- Availability heuristic
- Representativeness heuristic
- Anchoring
- Adjustment
- Recognition heuristic
- Affect heuristic
- Attribute substitution

### Biases and effects
- Framing effect
- Reference dependence
- Loss aversion
- Endowment effect
- Status quo bias
- Default effect
- Present bias
- Temporal discounting
- Hyperbolic discounting
- Probability weighting
- Base-rate neglect
- Conjunction fallacy
- Gambler's fallacy
- Hot-hand belief
- Overconfidence
- Calibration
- Confirmation bias
- Belief perseverance
- Self-serving bias
- Hindsight bias
- Outcome bias
- Omission bias
- Planning fallacy
- Optimism bias
- Escalation of commitment
- Sunk-cost effect
- Choice overload
- Decision fatigue
- Regret
- Anticipated regret
- Ambiguity aversion
- Risk perception

### Models and applications
- Rational choice models
- Expected utility theory
- Prospect theory
- Dual-process accounts
- Naturalistic decision-making
- Choice architecture
- Nudging
- Debiasing
- Decision errors
- Moral judgment and decision-making
- Group decision-making

---

# CHAPTER 1.11 — LANGUAGE AND INTELLIGENCE

### Language structure
- Language
- Symbolic communication
- Phoneme
- Phonology
- Morphology
- Syntax
- Semantics
- Pragmatics
- Lexical knowledge
- Lexical access
- Word recognition
- Speech perception
- Sentence processing
- Discourse processing
- Language comprehension
- Language production
- Speech production planning
- Reading
- Reading comprehension
- Writing

### Language development and variation
- Language acquisition
- Infant language learning
- Vocabulary development
- Grammar development
- Language development mechanisms
- Bilingualism
- Multilingualism
- Language and thought
- Linguistic relativity
- Sensitive period in language
- Language and memory
- Language and attention
- Language and executive control
- Language disorders
- Aphasia

### Intelligence
- Intelligence
- General intelligence
- g factor
- Fluid intelligence
- Crystallized intelligence
- Hierarchical intelligence models
- Cognitive abilities
- Reasoning ability
- Processing speed
- Working memory and intelligence
- Practical intelligence
- Multiple intelligences theory
- Emotional intelligence
- Intelligence testing
- IQ
- Standardization
- Reliability of intelligence tests
- Validity of intelligence tests
- Intelligence and heredity
- Intelligence and environment
- Intelligence and education
- Intelligence and problem solving
- Flynn effect
- Intelligence and creativity
- Individual differences in intelligence

---

# CHAPTER 1.12 — MOTIVATION, NEEDS, GOALS, AND SELF-REGULATION

### Motivation foundations
- Motivation
- Motive
- Need
- Drive
- Incentive
- Instinctive motivation
- Drive-reduction theory
- Arousal theory
- Incentive theory
- Need-based theories

### Goal systems
- Goal
- Goal representation
- Goal selection
- Goal activation
- Goal commitment
- Goal pursuit
- Goal monitoring
- Goal progress
- Goal standard
- Goal conflict
- Goal shielding
- Goal substitution
- Goal disengagement
- Goal reengagement
- Goal gradient

### Motivational forms
- Intrinsic motivation
- Extrinsic motivation
- Incentive motivation
- Approach motivation
- Avoidance motivation
- Achievement motivation
- Affiliation motivation
- Power motivation
- Curiosity
- Exploration
- Competence
- Autonomy
- Relatedness
- Need for achievement
- Need for affiliation
- Need for power

### Major models
- Self-determination theory
- Expectancy-value theory
- Control-value processes
- Regulatory focus
- Approach–avoidance regulation

### Biological and behavioral motives
- Hunger
- Appetite
- Satiety
- Thirst
- Sexual motivation
- Sexual desire
- Sexuality
- Sexual behavior
- Sexual orientation
- Reward
- Reward sensitivity
- Reward prediction
- Reward prediction error

### Self-regulation
- Self-regulation
- Self-efficacy
- Self-control
- Delay of gratification
- Impulse control
- Motivation regulation
- Effort
- Persistence
- Persistence failure
- Implementation intentions
- Action orientation
- State orientation
- Procrastination
- Motivation crowding
- Ego-depletion/resource models and evidence limits
- Motivation and context

---

# CHAPTER 1.13 — EMOTION AND AFFECT

### Core constructs
- Affect
- Mood
- Emotion
- Emotional episode
- Emotional stimulus
- Emotional valence
- Arousal
- Emotional intensity
- Physiological arousal
- Subjective feeling
- Emotional expression
- Action tendency
- Emotional awareness
- Emotional granularity
- Emotional differentiation

### Emotion processes
- Appraisal
- Emotion generation
- Emotion recognition
- Emotion perception
- Emotion regulation
- Emotional suppression
- Cognitive reappraisal
- Acceptance
- Expression regulation
- Display rules
- Emotional habits
- Emotion contagion

### Emotion theories
- James–Lange theory
- Cannon–Bard theory
- Two-factor theory
- Cognitive appraisal theories
- Basic-emotion approaches
- Dimensional approaches
- Constructed-emotion approaches
- Facial-feedback hypothesis

### Affective domains
- Positive affect
- Negative affect
- Happiness
- Sadness
- Fear
- Anger
- Disgust
- Surprise
- Interest
- Love and affection
- Shame
- Guilt
- Pride
- Envy
- Jealousy
- Gratitude
- Hope
- Anxiety as emotional experience

### Emotion interactions
- Emotion and cognition
- Emotion and memory
- Emotion and attention
- Emotion and decision-making
- Emotion and motivation
- Emotion and action
- Emotion and social judgment
- Emotional intelligence
- Individual differences in emotion

---

# CHAPTER 1.14 — PERSONALITY, SELF, IDENTITY, AND INDIVIDUAL DIFFERENCES

### Personality foundations
- Personality
- Personality trait
- Trait theory
- Personality state
- Personality process
- Trait activation
- Personality stability
- Personality change
- Personality development
- Person–situation debate
- Interactionist personality model

### Major perspectives
- Psychodynamic perspective
- Behavioral perspective
- Social-cognitive perspective
- Humanistic perspective
- Biological perspective
- Evolutionary perspective
- Trait perspective
- Cultural perspective

### Trait architecture
- Big Five model
- Openness
- Conscientiousness
- Extraversion
- Agreeableness
- Neuroticism
- Temperament
- Character
- Agency
- Communion

### Self
- Self
- Self-concept
- Self-schema
- Self-awareness
- Self-knowledge
- Self-esteem
- Self-worth
- Self-consistency
- Self-discrepancy
- Actual self
- Ideal self
- Ought self
- Possible selves
- Self-verification
- Self-enhancement
- Self-handicapping
- Self-presentation
- Impression management
- Self-monitoring

### Identity and individual differences
- Personal identity
- Identity structure
- Moral identity
- Personal values
- Locus of control
- Attributional style
- Optimism
- Pessimism
- Perfectionism
- Attachment style → 1.15 canonical owner (redirect only)
- Individual differences

### Assessment
- Personality assessment
- Self-report assessment
- Informant assessment
- Behavioral assessment
- Projective assessment
- Response distortion
- Social desirability
- Personality measurement validity

---

# CHAPTER 1.15 — HUMAN DEVELOPMENT ACROSS THE LIFESPAN

### Developmental science
- Development
- Lifespan development
- Developmental change
- Developmental mechanisms
- Developmental systems perspective
- Nature–nurture interaction
- Continuity
- Discontinuity
- Stability
- Change
- Critical period
- Sensitive period
- Developmental plasticity
- Cohort effect
- Developmental research methods

### Domains of development
- Biological development
- Motor development
- Perceptual development
- Cognitive development
- Language development
- Emotional development
- Social development
- Personality development
- Moral development
- Self development
- Identity development
- Executive-function development
- Emotion-regulation development
- Social-cognition development
- Theory of mind

### Lifespan stages
- Prenatal development
- Birth and neonatal development
- Infancy
- Early childhood
- Middle childhood
- Adolescence
- Puberty
- Emerging adulthood
- Adulthood
- Older adulthood
- Aging
- Cognitive aging
- Socioemotional aging
- Death and dying

### Developmental systems
- Attachment
- Attachment theory
- Attachment security
- Attachment insecurity
- Attachment development
- Temperament development
- Parenting processes
- Family influences
- Peer development
- School and learning context
- Cultural influences
- Adolescent identity exploration
- Adolescent risk-taking
- Adult cognitive development
- Socioemotional selectivity
- Cognitive reserve
- Developmental risk
- Developmental resilience

---

# CHAPTER 1.16 — SOCIAL PSYCHOLOGY AND SOCIAL COGNITION

### Social cognition
- Social psychology
- Social cognition
- Social perception
- Person perception
- Impression formation
- Attribution
- Internal attribution
- External attribution
- Attribution bias
- Fundamental attribution error
- Actor–observer difference
- Halo effect
- Horn effect
- False consensus effect
- Attitudes
- Attitude formation
- Attitude change
- Attitude–behavior relationship
- Social comparison
- Self-categorization

### Identity and groups
- Social identity
- Role identity
- Ingroup
- Outgroup
- Ingroup favoritism
- Outgroup homogeneity
- Social categorization
- Stereotype
- Prejudice
- Discrimination
- Implicit attitudes
- Stereotype threat
- Social identity threat
- Dehumanization
- Stigma

### Social influence
- Social influence
- Conformity
- Compliance
- Obedience
- Informational influence
- Normative influence
- Minority influence
- Social proof
- Social norms
- Norm enforcement
- Social roles
- Authority

### Groups
- Group processes
- Group cohesion
- Group polarization
- Groupthink
- Social facilitation
- Social loafing
- Deindividuation
- Collective behavior
- Group decision-making
- Leadership
- Status hierarchy
- Power dynamics
- Collective action
- Social network effects

### Relationships
- Interpersonal attraction
- Liking
- Affiliation
- Intimacy
- Attachment in adult relationships
- Love
- Commitment
- Relationship maintenance
- Relationship dissolution
- Social exchange
- Equity
- Interdependence
- Reciprocity
- Trust
- Trust development
- Trust repair
- Cooperation
- Competition
- Conflict

### Social behavior
- Aggression
- Prosocial behavior
- Helping
- Bystander effect
- Diffusion of responsibility
- Altruism
- Empathy
- Empathy–altruism relationship
- Moral behavior
- Moral judgment
- Moral emotions
- Emotional contagion
- Rejection
- Rejection sensitivity
- Ostracism
- Belonging
- Belonging uncertainty
- Loneliness
- Social exclusion
- Social threat
- Social support

---

# CHAPTER 1.17 — CULTURE AND SOCIOCULTURAL PSYCHOLOGY

### Culture
- Cultural psychology
- Culture
- Cultural model
- Cultural norm
- Cultural value
- Cultural schema
- Cultural script
- Cultural learning
- Enculturation
- Acculturation
- Biculturalism
- Bicultural identity
- Cultural frame switching
- Cultural adaptation
- Cultural mismatch
- Tight cultures
- Loose cultures
- Honor cultures

### Self and identity
- Cultural identity
- Independent self
- Interdependent self
- Individualism
- Collectivism
- Cultural models of self
- Cultural models of mind

### Sociocultural variation
- Cross-cultural differences
- Social class
- Socioeconomic status
- Gender
- Gender identity
- Sex differences
- Race
- Ethnicity
- Racial and ethnic identity
- Diversity
- Intersectionality
- Social identity intersections

### Culture across psychology
- Cultural variation in emotion
- Cultural variation in cognition
- Cultural variation in motivation
- Cultural variation in personality
- Cultural variation in social relationships
- Cultural variation in moral judgment
- Culture and perception
- Culture and development
- Culture and health
- Language and culture

### Methodology
- Cross-cultural research
- Measurement invariance across cultures
- WEIRD-sample limitation
- Generalizability across cultures
- Indigenous psychologies
- Global psychological perspectives

---

# CHAPTER 1.18 — STRESS, HEALTH, WELL-BEING, AND RESILIENCE

### Stress
- Stress
- Stressor
- Acute stress
- Chronic stress
- Stress appraisal
- Primary appraisal
- Secondary appraisal
- Challenge
- Threat
- Stress response
- Fight-or-flight response
- Physiological stress systems
- HPA-axis psychological relevance
- Stress recovery
- Allostatic load
- Allostatic processes

### Coping
- Coping
- Problem-focused coping
- Emotion-focused coping
- Approach coping
- Avoidant coping
- Cognitive coping
- Coping flexibility
- Coping effectiveness
- Social-support buffering

### Health
- Psychological health
- Physical health and behavior
- Health behavior
- Health beliefs
- Illness perceptions
- Health adherence
- Exercise and psychological function
- Nutrition and psychological function
- Sleep and health
- Substance use and health
- Chronic pain
- Pain appraisal
- Placebo effect
- Nocebo effect
- Behavioral medicine interface

### Well-being
- Well-being
- Subjective well-being
- Life satisfaction
- Hedonic well-being
- Eudaimonic well-being
- Meaning in life
- Purpose
- Flourishing
- Positive psychology
- Psychological flexibility
- Resilience
- Psychological recovery
- Post-stress adaptation
- Grief
- Bereavement
- Mourning
- Loss adaptation
- Posttraumatic growth — evidence-contested area

---

# CHAPTER 1.19 — PSYCHOLOGICAL DISORDERS AND ABNORMAL PSYCHOLOGY

### Foundations
- Abnormal psychology
- Psychological disorder
- Psychopathology
- Distress
- Dysfunction
- Clinical significance
- Diagnostic classification
- Categorical models
- Dimensional models
- Symptom dimensions
- Transdiagnostic processes
- Internalizing spectrum
- Externalizing spectrum
- Comorbidity

### Disorders
- Anxiety disorders
- Generalized anxiety disorder
- Panic disorder
- Phobias
- Social anxiety disorder
- Obsessive-compulsive disorder
- Obsessive-compulsive-related disorders
- Trauma- and stressor-related disorders
- Posttraumatic stress disorder
- Depressive disorders
- Major depressive disorder
- Bipolar disorders
- Schizophrenia spectrum disorders
- Psychosis
- Dissociative disorders
- Somatic symptom disorders
- Eating disorders
- Feeding disorders
- Substance use disorders
- Addiction and addictive behavior
- Behavioral addictions
- Gambling disorder
- Neurodevelopmental disorders
- Communication disorders
- Intellectual disability
- Specific learning disorders
- ADHD
- Autism spectrum disorder
- Motor disorders
- Tic disorders
- Tourette syndrome
- Personality disorders
- Psychopathy
- Sleep-wake disorders

### Clinical science
- Psychological assessment
- Clinical diagnosis
- Diagnostic reliability
- Diagnostic validity
- Risk factors
- Protective factors
- Biological factors in psychopathology
- Psychological factors in psychopathology
- Social and environmental factors
- Diathesis–stress models
- Stigma
- Recovery
- Suicide risk
- Self-harm
- Prevention
- Early intervention

---

# CHAPTER 1.20 — PSYCHOLOGICAL TREATMENT AND INTERVENTION SCIENCE

### Treatment science
- Psychological treatment
- Psychotherapy
- Treatment indication
- Treatment matching
- Treatment adherence
- Treatment engagement
- Treatment alliance
- Treatment outcome
- Treatment efficacy
- Treatment effectiveness
- Treatment mechanisms
- Common factors
- Context effects
- Placebo/contextual effects
- Treatment relapse
- Treatment maintenance

### Psychotherapy families
- Behavioral therapy
- Cognitive therapy
- Cognitive-behavioral therapy
- Exposure-based therapy
- Third-wave behavioral therapies
- Humanistic therapy
- Psychodynamic therapy
- Family/systemic therapy
- Interpersonal therapy
- Group psychotherapy

### Biological and combined treatment
- Pharmacological treatment
- Biological treatments
- Combined treatment
- Stepped care
- Prevention intervention
- Early intervention

### Evidence and ethics
- Evidence-based practice
- Treatment evidence
- Treatment guidelines
- Clinical decision-making
- Treatment risk–benefit
- Informed consent in treatment
- Therapist competence
- Therapeutic boundaries
- Confidentiality
- Measurement-based care
- Routine outcome monitoring
- Cultural adaptation of treatment

---

# CHAPTER 1.21 — HUMAN BEHAVIOR, ACTION, AND ACTION CONTROL

This chapter is deliberately rebuilt. It does NOT re-own motivation, learning, reinforcement, habit formation, or personality.

### Action architecture
- Behavior
- Action
- Behavioral intention
- Action selection
- Action initiation
- Action execution
- Action monitoring
- Action outcome
- Action correction
- Motor control
- Behavioral activation
- Behavioral inhibition
- Action error
- Error monitoring
- Action–outcome learning
- Goal-directed action
- Habitual action
- Automatic action
- Controlled action

### Behavioral regulation
- Behavioral regulation
- Behavioral flexibility
- Behavioral rigidity
- Behavioral adaptation
- Behavioral persistence
- Effort allocation
- Behavioral choice
- Environment–behavior coupling
- Behavior–consequence relation
- Person–situation behavior

### Performance
- Performance
- Skill performance
- Expertise performance
- Deliberate practice
- Feedback-based improvement
- Performance regulation
- Performance adaptation
- Behavioral individual differences

### Explicit redirects
The following are NOT independent owners here:
- Reinforcement → 1.7
- Motivation → 1.12
- Self-control → 1.12
- Habit formation → 1.7
- Personality → 1.14
- Social behavior → 1.16
- Skill learning → 1.7
- Emotion regulation → 1.13

---

# II. CROSS-CHAPTER RELATIONSHIP ARCHITECTURE

Relationships are represented as links, not duplicate canonical concepts.

Examples:
- Attention ↔ perception
- Attention ↔ working memory
- Emotion ↔ attention
- Emotion ↔ memory
- Motivation ↔ emotion
- Motivation ↔ decision-making
- Learning ↔ memory
- Learning ↔ action
- Personality ↔ behavior
- Personality ↔ situation
- Development ↔ cognition
- Culture ↔ cognition
- Culture ↔ emotion
- Stress ↔ cognition
- Stress ↔ health
- Social influence ↔ decision-making
- Social identity ↔ personality
- Biology ↔ every psychological domain

---

# III. THEORY / MODEL REGISTRY

The following are models, not duplicate constructs:

- Biopsychosocial model
- Dynamic-systems perspective
- Information-processing approaches
- Dual-process accounts
- Prospect theory
- Expected utility theory
- Rational choice models
- Self-determination theory
- Expectancy-value theory
- Regulatory focus theory
- Drive-reduction theory
- Arousal theory
- Incentive theory
- James–Lange theory
- Cannon–Bard theory
- Two-factor theory
- Cognitive appraisal theories
- Basic-emotion approaches
- Dimensional emotion models
- Constructed-emotion approaches
- Social-cognitive theory
- Trait models
- Psychodynamic models
- Humanistic models
- Attachment models
- Developmental systems approaches
- Diathesis–stress model
- Stress appraisal models
- Health behavior models

Each theory must state:
1. What it explains.
2. Core assumptions.
3. Evidence.
4. Limitations.
5. Competing explanations.
6. Current scientific status.
7. Canonical constructs it connects.

---

# IV. APPLICATION BOUNDARY

The following remain outside System 1 unless a universal psychological mechanism is being explained:

- Investor psychology
- Sales psychology
- Cold calling
- Communication engineering
- Persuasion methodology
- Negotiation methodology
- Buyer psychology
- Objection handling
- Relationship selling
- Sales performance systems
- Brokerage operations
- Organizational implementation
- Vocational application
- AI implementation

They may use System 1 constructs but cannot re-own them.


---

# VI. SURGICAL AUDIT RESOLUTION — v2.1

## 1. Coverage corrections

The audit identified and resolved the following genuine coverage gaps:

- **Self-efficacy** is now explicitly owned by Chapter 1.12 because it is a general motivational/self-regulatory construct with broad behavioral relevance. APA describes self-efficacy as a belief about one's capability that influences motivation, emotion, and behavior change. citeturn1search2turn1search6
- **Attachment** is now explicitly owned by Chapter 1.15. Adult attachment remains a developmental/social manifestation linked to that owner rather than a second personality owner. APA's current attachment material treats attachment processes across infancy through adulthood. citeturn1search0turn1search3
- **Sexuality, sexual desire, sexual behavior, and sexual orientation** are explicitly represented as universal psychological topics. Their social, cultural, developmental, and clinical manifestations are linked rather than duplicated. APA treats sexuality as a major psychological domain involving desire, behavior, identity/orientation, development, and context. citeturn2search5turn2search19
- **Grief, bereavement, mourning, and loss adaptation** are now represented in Chapter 1.18. Normal grief is kept distinct from prolonged grief disorder, which remains a clinical condition in Chapter 1.19. APA currently treats grief as a distinct psychological process and distinguishes it from prolonged grief disorder. citeturn4search9turn4search1
- **Addiction and addictive behavior** and **gambling disorder** are now explicit in Chapter 1.19. APA materials treat addiction and addictive behaviors as established psychological/clinical domains, including behavioral addictions. citeturn1search4turn1search17
- **Measurement-based care** and **routine outcome monitoring** are now explicit in Chapter 1.20. These belong to intervention science, not to the general measurement chapter. APA's evidence-based practice framework emphasizes monitoring, clinical decision-making, treatment evidence, and adaptation to patient characteristics and context. citeturn3search0turn3search2

## 2. Ownership corrections

The following duplicate-owner conflicts are resolved by canonical redirects:

| Object | Canonical owner | Non-owner appearances |
|---|---|---|
| Behavior | 1.1 | 1.21 uses behavioral/action derivatives |
| Action | 1.21 | 1.1 uses it only in processing architecture |
| Individual differences | 1.14 | other chapters use domain-specific manifestations |
| Self-regulation | 1.12 | 1.1 and other chapters link to it |
| Psychological flexibility | 1.18 | 1.1 links to the construct |
| Self-awareness | 1.5 | 1.14 links to self-related manifestations |
| Bounded rationality | 1.10 | 1.9 links to the decision construct |
| Dynamic-systems perspective | 1.1 | development chapters link to it |
| Emotional intelligence | 1.13 | intelligence chapter links to it as a cross-domain construct |
| Attachment | 1.15 | 1.14 and 1.16 use developmental/relationship manifestations |
| Personality development | 1.15 | 1.14 links to personality as a domain |
| Confidentiality | 1.2 as research ethics; 1.20 contains treatment-ethics application only | no second general owner |

## 3. Relationship-node rule

The following are **not independent canonical owners** even when they appear as useful chapter entries:

- X and Y
- X–Y relationship
- X and behavior
- X and memory
- X and attention
- X and decision-making
- cultural variation in X
- individual differences in X
- X in context
- X and health
- X and development
- X and learning
- X and intelligence
- X and personality
- X and social behavior
- X and psychological function

These are relationship/application nodes. Their content is retrieved through the canonical owners on both sides.

This resolves the previous error where cross-domain relationships were being counted as independent psychological concepts.

## 4. Theory/model separation

Theories and models remain canonical knowledge objects where they explain a major psychological domain, but they are no longer counted as if they were basic constructs.

Canonical model ownership follows the primary explanatory target:

- learning models → 1.7
- decision models → 1.10
- motivation models → 1.12
- emotion models → 1.13
- personality models → 1.14
- developmental models → 1.15
- psychopathology models → 1.19
- stress models → 1.18
- treatment/evidence models → 1.20
- biopsychosocial and general systems models → 1.1

## 5. Clinical separation

Clinical conditions are not treated as ordinary constructs.

Chapter 1.19 owns:

- diagnostic categories
- symptom dimensions
- clinical syndromes
- risk/protective factors
- comorbidity
- clinical assessment
- stigma and recovery
- suicide risk and self-harm
- addiction and behavioral addictions
- prolonged grief disorder

General mechanisms remain in their universal owners and are only linked into clinical chapters.

## 6. Developmental and cultural separation

Developmental and cultural chapters own developmental/cultural phenomena and manifestations, but they do not re-own the general psychological mechanism.

Examples:

- memory remains 1.8; memory in development is a developmental link.
- emotion remains 1.13; cultural emotion is a cultural manifestation.
- motivation remains 1.12; developmental motivation is a developmental application.
- personality remains 1.14; cultural personality differences are a cultural manifestation.
- attachment remains 1.15; adult attachment is a lifespan manifestation.

This preserves one-owner retrieval while allowing deep contextual coverage.

## 7. Cross-system collision audit

The following remain outside System 1 as independent owners:

- sales psychology
- investor psychology
- brokerage psychology/operations
- communication engineering
- conversation engineering
- persuasion methodology
- negotiation methodology
- buyer psychology
- objection handling
- relationship selling
- sales performance systems
- organizational implementation
- vocational application
- AI implementation

System 1 supplies universal mechanisms to these systems; it does not absorb their domain-specific methods.

## 8. Final architecture count

The v2 architecture contains:

- **21 chapters**
- **1,364 chapter architecture entries**
- **29 model-registry entries**
- **18 explicit cross-chapter relationship links**
- **14 application-boundary entries**

After canonical ownership resolution, relationship/application entries and duplicate aliases are excluded from the independent-owner count.

### FINAL CANONICAL OWNER COUNT: 1,284

This count is the count of independently owned canonical knowledge objects, not the raw number of visible bullets.

The difference is deliberate:

> **The map may contain links, manifestations, redirects, and models without turning each one into a new owner.**

## 9. Lock-gate result

| Gate | Result |
|---|---|
| Coverage audit | PASS |
| Every canonical leaf has an ontology type | PASS |
| One canonical owner per construct | PASS |
| Relationship nodes removed from concept count | PASS |
| Theories/models separated from constructs | PASS |
| Clinical conditions separated from general mechanisms | PASS |
| Developmental duplication controlled | PASS |
| Cultural duplication controlled | PASS |
| Applied-domain collision controlled | PASS |
| Major established psychology domains have a home | PASS |
| Missing-concept search resolved | PASS |
| Duplicate scan | PASS after canonical redirects |
| Cross-system collision scan | PASS |
| Final canonical count generated | PASS — 1,267 |
| Prose-writing gate | **OPEN** |

## 10. Scientific basis of the audit

The coverage audit was checked against current mainstream psychology frameworks rather than relying only on the old index. APA's current National Standards place scientific inquiry/research methods at the foundation and organize core psychology across biological, cognition, development/learning, social/personality, and mental/physical health areas. citeturn0search0turn0search33

OpenStax Psychology 2e independently covers research, biopsychology, consciousness, sensation/perception, learning, thinking/intelligence, memory, lifespan development, motivation/emotion, personality, social psychology, stress/health, disorders, and therapy/treatment. citeturn0search1

APA's current research-methods material also supports explicit treatment of causal inference, qualitative and quantitative designs, measurement, psychometrics, ethics, transparency, preregistration, and open science. citeturn0search3turn0search5turn3search4turn3search5

### Lock decision

**SYSTEM 1 — HUMAN PSYCHOLOGY — SURGICAL CANONICAL ARCHITECTURE v2.1 IS LOCKED.**

No prose chapter generation is authorized by this architecture document until the writing constitution is applied to the locked owner map.

---

# V. FINAL ARCHITECTURAL GATE

System 1 may be permanently locked only after:

1. Every chapter passes coverage audit.
2. Every leaf has an ontology type.
3. Every leaf has one owner.
4. Relationship nodes are removed from the concept count.
5. Theories/models are separated from constructs.
6. Clinical conditions are separated from general mechanisms.
7. Cultural and developmental manifestations do not duplicate general owners.
8. Applied domains remain outside the universal foundation.
9. Every major established psychology domain has an explicit home.
10. Every major missing-concept search returns either an owner or an explicit justified exclusion.
11. A duplicate scan passes.
12. A cross-system collision scan passes.
13. A final concept count is generated.
14. Only then is the index marked LOCKED.

**PROSE WRITING IS PROHIBITED UNTIL THIS GATE PASSES.**
